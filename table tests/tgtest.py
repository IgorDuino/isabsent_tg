...
def callback(call: types.CallbackQuery):
    ...
    if data == 'student':
        ...
        bot.register_next_step_handler(msg, process_name_step)
    elif data.startswith('iam_'):
        data = data[4:]
        user_dict[user_id].name = ' '.join(data.split()[:3])
        user_dict[user_id].class_ = data.split()[3]
...
def process_name_step(message):
    ...
    if len(find_res) == 0:
        ...
        for student in find_res:
            keyboard.add(types.InlineKeyboardButton(text=student[0], callback_data='iam_' + student[0]))
